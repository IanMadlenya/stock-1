...
public class NewsTokenizer {
	...
	str = str.toLowerCase();
        ...
        str = str.replaceAll("[0-9]+(,[0-9]+)*(.[0-9]+)?\\s+", " ");
        ...
        str = str.replaceAll("(" + regexClitics + ")", " ");
        ...
        for (String word : words) {
        	...
        	if (!stopwords.contains(word.substring(0, word.length() - 1)) && !isNumber(word.substring(0, word.length() - 1)) && !abbrList.contains(word.substring(0, word.length() - 1))) {
        	    tokenList.add(word.substring(0, word.length() - 1));
        	    //tokenList.add(word.substring(word.length() - 1));
        	}
            } else {
        	if (!stopwords.contains(word) && !isNumber(word) && !abbrList.contains(word)) {
        	    tokenList.add(word);
        	}
            }
        }
